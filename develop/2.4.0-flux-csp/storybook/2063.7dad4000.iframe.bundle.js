...
`;var h=t("../../libs/components/src/form/select-rich/vl-select-rich.defaults.ts");class _ extends a.M{constructor(){super(),this.options=h.X.options,this.initialOptions=h.X.initialOptions,this.placeholder=h.X.placeholder,this.search=h.X.search,this.searchPlaceholder=h.X.searchPlaceholder,this.choices=null,this.notDeletable=h.X.notDeletable,this.multiple=h.X.multiple,this.position=h.X.position,this.resultLimit=h.X.resultLimit,this.noResultsText=h.X.noResultsText,this.noChoicesText=h.X.noChoicesText,this.value=null,this.dropdownInitialised=!1,this.isDropdownOpen=!1,this.dispatchInput=!1,this.callbackOnInit=()=>{this.getChoicesElement()?.addEventListener("click",this.onClickChoices),this.internals.labels[0]?.addEventListener("click",this.onClickChoices),this.getChoicesElement()?.addEventListener("showDropdown",()=>{let e=this.getChoicesElement()?.querySelector(".vl-select__list--dropdown");e&&!this.dropdownInitialised&&(e.setAttribute("role","group"),e.setAttribute("id","vl-select__list"),this.dropdownInitialised=!0),this.isDropdownOpen=!0}),this.getChoicesElement()?.addEventListener("hideDropdown",()=>{this.isDropdownOpen=!1}),this.setChoicesInputAttributes()},this.onClickChoices=e=>{e.stopPropagation(),this.disabled||this.choices?.showDropdown()},this.onSearchInput=e=>{let l=e?.target?.value;this.dispatchEvent(new CustomEvent("vl-select-search",{bubbles:!0,composed:!0,detail:{value:l}}))},this.submitFormOnEnter=!1}static get styles(){return[s.h8,s.BI,d.N,v,u,o.py,m]}static get properties(){return{initialOptions:{type:Array,attribute:"initial-options"},options:{type:Array},placeholder:{type:String},notDeletable:{type:Boolean,attribute:"not-deletable"},multiple:{type:Boolean},search:{type:Boolean},position:{type:String},resultLimit:{type:Number,attribute:"result-limit"},noResultsText:{type:String,attribute:"no-results-text"},noChoicesText:{type:String,attribute:"no-choices-text"},searchPlaceholder:{type:String,attribute:"search-placeholder"},value:{type:FormData,state:!0,hasChanged:(e,l)=>e instanceof FormData&&l instanceof FormData?JSON.stringify([...e.entries()])!==JSON.stringify([...l.entries()]):e!==l}}}firstUpdated(e){super.firstUpdated(e),this.choices=new r.A(this.validationTarget,this.getChoicesConfig()),this.initialOptions=structuredClone(this.options),setTimeout(()=>{this.value||this.setValue(null),this.choices?.input?.element?.addEventListener("input",this.onSearchInput)},0)}updated(e){if(super.updated(e),this.choices){if(e.has("options")&&(this.choices.clearStore(),this.choices.setChoices(this.options,"value","label",!0),this.updateSelectedOptions(this.options)),e.has("value")){let e={value:this.getSelected()};this.setValue(this.value),this.dispatchEvent(new CustomEvent("vl-change",{bubbles:!0,composed:!0,detail:e})),this.validity.valid&&this.dispatchEventIfValid(e)}e.has("disabled")&&(this.disabled?this.choices.disable():this.choices.enable()),e.has("error")&&this.internals.setValidity({customError:this.error},"custom-error"),e.has("resultLimit")&&(this.choices.config.searchResultLimit=this.resultLimit)}}disconnectedCallback(){super.disconnectedCallback(),this.getChoicesElement()?.removeEventListener("click",this.onClickChoices),this.internals.labels[0]?.removeEventListener("click",this.onClickChoices),this.choices?.input?.element?.removeEventListener("input",this.onSearchInput),this.choices?.destroy()}render(){let e={"vl-select":!this.multiple,"vl-multiselect":this.multiple,"vl-select--disabled":this.disabled,"vl-select--error":this.isInvalid||this.error,"vl-select--success":this.success};return(0,c.qy)`
            <select
                id=${this.id||c.s6}
                name=${this.name||c.s6}
                class=${(0,n.H)(e)}
